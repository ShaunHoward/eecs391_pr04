...
/**
 * A reinforcement learning agent designed to learn how to use the good footmen
 * to defeat the enemy footmen in SEPIA game engine.
 * 
...
public class RLAgent extends Agent {
	...
	private AttackAction prevAction = new AttackAction(
	...
	public RLAgent(int playernum, String[] args) {
		...
		if (args.length > 0) {
			...
			episodes = 10;
			System.out
					.println("Warning! Number of episodes not specified. Defaulting to 10 episodes.");
		}
		...
		if (args.length > 1) {
			...
			System.out
					.println("Warning! Load weights argument not specified. Defaulting to not loading.");
		}
	...
	/**
	 * Calculates the reward at each step and updates the totals. Checks if an
	 * event has occurred. If it has then you weights are updated and a new
	 * action is selected.
	 *
	...
	public Map<Integer, Action> middleStep(StateView newState,
		...
		State currentState = new State(curFootmen, curEnemyFootmen,
	 ...
	 */
	public static double[] calculateFeatureVector(State state, Integer footman,
	 ...
	 * @return the feature vector in reference to the given values
	 */
	public static double[] getFeatures(Integer footman, Integer enemy,
	...
	/**
	 * Updates the feature weights vector given a feature vector, a calculated
	 * loss function, and an alpha.
	 * 
	 * @param f
	 *            - the feature vector to update the feature weights set with
	 * @param calcLoss
	 *            - the calculated loss function
	 * @param alpha
	 *            - a variable alpha value
	 */
	...
	/**
	 * Determines if a significant event has happened, which is someone being
	 * attacked or someone dying.
	 * 
	 * @param curState
	 * @param prevState
	...
	private boolean eventHasHappened(State curState, State prevState) {
		if (curState.footmenDeadCount > prevState.footmenDeadCount
				|| curState.getEnemyFootmen().size() < prevState
						.getEnemyFootmen().size()) {
			// uh oh, they dead
			return true;
		}
	 ...
	 * @param footman
	 */
	private void updateWeights(double reward, State curState, State prevState,
			AttackAction prevAction, Integer footman) {
		State currentState = new State(curState);
		double[] previousFeatures = calculateFeatureVector(prevState, footman,
				prevAction.getAttack().get(footman), prevAction);
		...
		if (!currentState.getFootmen().contains(footman)) {
			...
			currentState.getUnitLocations().put(footman,
					prevState.getUnitLocations().get(footman));
		}
	 ...
	 * @return The reward
	 */
	private double calculateReward(State curState, State prevState,
			AttackAction prevAction, Integer footman) {
		...
		curState.getUnitLocations().put(
				prevAction.getAttack().get(footman),
				prevState.getUnitLocations().get(
						prevAction.getAttack().get(footman)));
		...
		if (!curState.getFootmen().contains(footman)) {
			...
			// Ally injured
			int healthLost = prevState.getUnitHealth().get(footman)
		...
		if (!curState.getEnemyFootmen().contains(target)) {
			curState.getUnitLocations().put(target,
					prevState.getUnitLocations().get(target));
		}

		// (EHP/EKILLED) Update reward based on enemy's health.
		if (areAdjacent(curState.getUnitLocations().get(footman), curState
			...
			if (!curState.getEnemyFootmen().contains(target)) {
				...
				// Enemy injured
				int healthLost = prevState.getUnitHealth().get(target)
	 ...
	 */
	// public double calculateReward(StateView stateView, History.HistoryView
	// historyView, int footmanId) {
	// return 0;
	// }
